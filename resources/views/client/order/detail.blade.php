            ...
            <div class="row">
                <div class="col-lg-8">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            @if($values['order']->tracking == 4)
                                <div class="text-danger">Order cancel by admin</div>
                            @elseif($values['order']->tracking == 5)
                                <div class="text-danger">Order cancel by you</div>
                            @else
                    ...
                    @if($values['order']->tracking == 2)
                        <div class="card mb-4">
                            <div class="card-header text-black" style="background: #fff; border-radius: 10px 10px 0 0">Order received</div>
                            <div class="card-body">
                                <form action="{{ route('client.order.update.tracking', $values['order']->order_code) }}" method="post">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="tracking" value="3">
                    ...
                    @if($values['order']->tracking == 0)
                        <div class="card mb-4">
                            <div class="card-header text-black" style="background: #fff; border-radius: 10px 10px 0 0">Cancel Order</div>
                            <div class="card-body">
                                <form action="{{ route('client.order.update.tracking', $values['order']->order_code) }}" method="post">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="tracking" value="5">
                ...
                <div class="col-lg-4">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            @if ($values['order']->carriers != null)
                                <hr>
                                    <h3 class="h6">Shipping Information</h3>
                                    <strong>{{ $values['order']->carriers['name'] }}</strong>
                                    <span><a href="https://jtexpress.vn/vi/tracking?type=track&billcode={{ $values['order']->carriers['billcode'] }}" class="text-decoration-underline" target="_blank">1298376192873612</a> <i class="bi bi-box-arrow-up-right"></i> </span>
                            @endif
                            <hr>
                            ...
                            <ul class="list-group list-group-flush">
                                ...
                                @if($values['order']->promotion != null)
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <span class="text-danger text-end"><del>-{{ $values['order']->getDiscountFormatAttribute() }}</del></span>
                                    </li>
                                @endif
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <strong>TOTAL</strong>
